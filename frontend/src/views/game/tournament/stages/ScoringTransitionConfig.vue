<template>
  <div class="space-y-6">
    ...
    <div class="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
      <h3 class="text-sm font-bold text-neutral-400 uppercase tracking-wider mb-4 flex items-center gap-2"><Target class="w-4 h-4" /> 打分配置</h3>
      <div class="space-y-4">
        <div>
        ...
        <template v-else-if="localScoring.type === 'MULTI_DIM'">
          ...
          <div>
            ...
            <div v-if="!localScoring.dimensions || localScoring.dimensions.length === 0" class="text-xs text-neutral-600">
              未配置维度(将按单一总分处理)
            </div>
            <div v-for="(d, i) in localScoring.dimensions" :key="i" class="grid grid-cols-12 gap-2 mb-2">
              ...
              <input
                ...
                placeholder="权重"
                :disabled="localScoring.aggregateRule !== 'WEIGHTED'"
                class="cfg-input col-span-3 disabled:opacity-40"
                @input="onScoringChange"
              />
...
const props = defineProps<{ scoring?: any }>();
const emit = defineEmits<{ 'update:scoring': [any] }>();
...
watch(
  () => props.scoring,
  (v) => {
    if (v) localScoring.value = clone(v);
  }
);
</script>
