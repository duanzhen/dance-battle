<template>
  <div class="w-full h-full">
    <!-- 查看模式:顶部顺序列表 + 中间当前对决 -->
    <div v-if="mode !== 'edit'" ref="viewRef" class="w-full h-full relative">
      ...
      <div v-else-if="!stageId" class="w-full h-full flex items-center justify-center text-white/40" :style="{ fontSize: sz(12) + 'px' }">
        未绑定擂台赛段
      </div>
      <div v-else class="w-full h-full flex flex-col">
        <!-- 顶部顺序列表:按队列顺序一行展示 头像/名字/积分,队首为擂主 -->
        <div class="flex-none px-3 pt-3">
          <div class="flex flex-nowrap items-stretch justify-between" :style="{ gap: sz(10) + 'px' }">
            <div
              v-for="c in queue"
              :key="c.competitorId"
              class="relative flex-1 min-w-0 rounded-lg border p-2 pt-3 flex flex-col items-center gap-1"
              :class="
                c.queueIndex === 1 ? 'border-amber-400/70 bg-amber-500/10' : isInBattle(c.competitorId) ? 'border-green-500/40' : 'border-white/10'
              "
            >
              <div class="relative">
                <img
                  ...
                  :style="{ width: sz(68) + 'px', height: sz(68) + 'px' }"
                  :class="c.queueIndex === 1 ? 'border-amber-400/80' : 'border-white/15'"
                  @error="onImgError"
                />
                <div
                  v-else
                  class="rounded-full bg-white/5 border-2"
                  :style="{ width: sz(68) + 'px', height: sz(68) + 'px' }"
                  :class="c.queueIndex === 1 ? 'border-amber-400/80' : 'border-white/15'"
                ></div>
                <span
                  v-if="c.queueIndex === 1"
                  class="absolute -right-1 -top-1 rounded-full bg-amber-500 flex items-center justify-center"
                  :style="{ width: sz(24) + 'px', height: sz(24) + 'px' }"
                >
                  <Crown class="text-neutral-900" :style="{ width: sz(14) + 'px', height: sz(14) + 'px' }" />
                </span>
              </div>
              <div class="w-full text-center min-w-0">
                <div class="text-white font-bold truncate" :style="{ fontSize: sz(15) + 'px' }" :title="c.name">{{ c.name || '待定' }}</div>
                <div class="text-amber-400 font-mono leading-tight" :style="{ fontSize: sz(17) + 'px' }">{{ c.points ?? 0 }}</div>
              </div>
            </div>
            <div v-if="!queue.length" class="text-white/30 py-6 px-2 flex-none" :style="{ fontSize: sz(13) + 'px' }">暂无参赛者</div>
          </div>
        </div>
        ...
        <div class="flex-1 min-h-0 flex items-center justify-center px-6 py-3">
          <div v-if="currentMatch || queue.length >= 2" class="w-full h-full flex items-center justify-center gap-[6%]">
            <div class="flex flex-col items-center gap-2 flex-1 min-w-0 h-full justify-center">
              ...
              <span class="text-white font-bold text-[clamp(14px,2vw,30px)] truncate max-w-full" :title="defender.name">{{
                defender.name || '待定'
              }}</span>
            ...
            <div class="flex flex-col items-center gap-2 flex-1 min-w-0 h-full justify-center">
              ...
              <div v-else class="h-[58%] w-full"></div>
              <span class="text-white font-bold text-[clamp(14px,2vw,30px)] truncate max-w-full" :title="challenger.name">{{
                challenger.name || '待定'
              }}</span>
    ...
    <div v-else class="space-y-4 px-2 py-4">
      <section>
        ...
        <p class="text-[10px] text-neutral-600 mt-2">
          顶部一行按轮转顺序展示参赛者（头像/名字/积分，队首为擂主），中间展示当前对决；结算由现场决定，系统只负责累计积分。
        </p>
      </section>
    </div>
  </div>
...
/** 组件内元素缩放:以 800x600 为基准,大组件放大、小组件缩小,限制 0.5~2.5 */
const scale = ref(1);
