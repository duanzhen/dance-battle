...
class KnockoutGeneratorTest {
    ...
    void fivePlayers_producesByes() {
        ...
        assertEquals(4, plan.getMatches().size(), "每轮一赛段:8 人只生成首轮 4 场");
        List<MatchPlan> r1 = round(plan, 1);
        // 摆位 [1,8,5,4,3,6,7,2] → BYE 在 seed 6,7,8 → 3 个 BYE 槽
    ...
    @Test
    void knockout_onlyFirstRound_allFinalMatch() {
        // 淘汰赛每轮一赛段:8 人只生成首轮 4 场,每场 finalMatch(胜者全晋级下一赛段)
        RuleConfigHolder rc = new RuleConfigHolder();
        KnockoutConfig kc = new KnockoutConfig();
        rc.setKnockout(kc);
        ...
        assertEquals(8, plan.getBracketSize());
        assertEquals(4, plan.getMatches().size(), "应只有首轮 4 场");
        // 全部为 round 1,无后续轮
    ...
    @Test
    void knockout_seedPairing_topVsBottom() {
    ...
    @Test
    void fourPlayers_thirdPlaceMatch_generatesLoserMatch() {
        // 4 人半决赛(每轮一赛段)开启季军赛:两场半决赛 + 季军赛(3 场),败者路由到季军赛
        RuleConfigHolder rc = new RuleConfigHolder();
        KnockoutConfig kc = new KnockoutConfig();
    ...
    void fourPlayers_thirdPlaceDisabled_noExtraMatch() {
        // 4 人半决赛未开启季军赛:只生成 2 场半决赛,无季军赛、无败者路由
        RuleConfigHolder rc = new RuleConfigHolder();
        KnockoutConfig kc = new KnockoutConfig();
        kc.setThirdPlaceMatch(false);
        rc.setKnockout(kc);

        List<Long> seeds = List.of(1L, 2L, 3L, 4L);
        BracketPlan plan = generator.generate(seeds, rc);

        assertEquals(2, plan.getMatches().size(), "4 人应为 2 场半决赛");
